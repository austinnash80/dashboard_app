class EmpireMasterMoMatchesController < ApplicationController
  ...
  def run
    ...
    master = EmpireMasterMoList.pluck(:lic, :lname)
    customer = EmpireMember.where.not(uid: already_matched_uid).where(state: 'MO').pluck(:lic_num, :lname)
    ...
    EmpireMember.where(state: 'MO').where(lic_num: lic).each do |i|
