class MasterEaNoMatchesController < ApplicationController
  before_action :set_master_ea_no_match, only: %i[ show edit update destroy ]

  # GET /master_ea_no_matches or /master_ea_no_matches.json
  def index
    ...
    if params['remove_all'] == 'yes' && params['confirm'] == 'yes'
      ...
      redirect_to master_ea_no_matches_path(), notice: 'Records Deleted'
    end
    # EXPORT
    respond_to do |format|
      format.html
      format.csv { send_data @master_ea_no_matches.to_csv, filename: "Sequoia-Master-EA-No-Matches-#{Date.today}.csv" }
    end
  end

  # GET /master_ea_no_matches/1 or /master_ea_no_matches/1.json
  def show
  end
  ...
  # POST /master_ea_no_matches or /master_ea_no_matches.json
  def create
  ...
  # PATCH/PUT /master_ea_no_matches/1 or /master_ea_no_matches/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /master_ea_no_matches/1 or /master_ea_no_matches/1.json
  def destroy
  ...
  private
    ...
    def master_ea_no_match_params
      params.require(:master_ea_no_match).permit(:uid, :list, :lname, :search_date)
    end
end
