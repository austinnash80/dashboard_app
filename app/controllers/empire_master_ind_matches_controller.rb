class EmpireMasterIndMatchesController < ApplicationController
  ...
  def run
    member = EmpireMember.where(state: "IND").pluck(:lic_num)
    master = EmpireMasterIndList.pluck(:lic)
    matched = EmpireMasterIndMatch.pluck(:lic)
    new = (master - matched) & member
    list = EmpireMasterIndList.first(1).pluck(:list)[0]

    if params['type'] == 'large'
      EmpireMasterIndList.select("id","lid","lic", "exp_date", "lname").where(lic: new).find_in_batches(batch_size: 500).each do |masters|
      ...
      EmpireMasterIndMatch.where(uid: nil).each do |i|
        empire_member = EmpireMember.where(state: 'IND').find_by(lic_num: i.lic)
        master_list = EmpireMasterIndList.find_by(lic: i.lic)
        EmpireMasterIndMatch.where(id: i.id).update_all lid: master_list.lid, exp: master_list.exp_date, lname: master_list.lname, uid: empire_member.uid
      end
    end

    expired = EmpireMember.where(state: 'IND').where(lic_expired: true).count
    other = EmpireMember.where(state: 'IND').where(lic_not_found: true).count + EmpireMember.where(state: 'IND').where(lic_not_in_master: true).count
    total = EmpireMember.where(state: 'IND').count
    matched = EmpireMasterIndMatch.count
    EmpireState.where(st: 'IND').update_all customers: total, matched_customers: matched, lic_expired: expired, lic_other: other
